...
int pageComparator(uint32_t pageA, uint32_t pageB) {
...
int sortNumbers(const void *aVoid, const void *bVoid) {
    uint32_t pageA = *(uint32_t *)aVoid;
    uint32_t pageB = *(uint32_t *)bVoid;

    return pageComparator(pageA, pageB);
}

bool isCorrectOrder(Uint32Vector *pages) {
    ...
    for (size_t first = 0; first < pageCount - 1; first++) {
        for (size_t second = first + 1; second < pageCount; second++) {
            ...
            int correctOrder = pageComparator(firstPage, secondPage);
            if (correctOrder == -1) {
                break;
            }
            if (correctOrder == 1) {
...
int main(int argc, char **argv) {
    ...
    while ((line = readLineFromFile(input, '\n')) != NULL) {
        ...
        if (!isCorrectOrder(&pages)) {
            uint32_t *pageArray = vector_uint32_data(&pages);
            qsort(pageArray, pageCount, sizeof(uint32_t), sortNumbers);
